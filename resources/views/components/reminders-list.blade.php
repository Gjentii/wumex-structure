@php
    ...
    $record = $getRecord();
    $reminders = $record ? $record->reminders : collect();
@endphp
...
{{--            <div>--}}
{{--                <h1>{{ $reminder->name }}</h1>--}}
{{--                <p>Expires on: {{ $reminder->expires_on->format('Y-m-d') }}</p>--}}
{{--            </div>--}}
{{--        @endforeach--}}
{{--    @endif--}}
{{--</div>--}}
...
{{--                                        </x-filament::dropdown.list.item>--}}
                                        <hr>
                                        <x-filament::dropdown.list.item color="danger" wire:click="detachReminder({{ $reminder->id }})">
                                            <p class="text-red-600 !important">Remove contact from this item</p>
                                        </x-filament::dropdown.list.item>
                                    </x-filament::dropdown.list>
                                </x-filament::dropdown>
                            </div>
                        ...
                        <p class="text-sm text-gray-500 truncate dark:text-gray-400">
                            Expires on: {{ $reminder->expires_on->format('Y-m-d') }}
                        </p>
                    </div>
                </div>
            </div>
